...
const ChatBox = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`/api/message/${mode}`, {
                chatId:
                    selectedChat._id, prompt, isPublished
    ...
    useEffect(() => {
        if (selectedChat?.messages) { // ✅ safe check
    ...
    return (
        <div className="flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-32 max-md:mt-14 2xl:pr-20">

            {/* Chat Messages */}
            <div ref={containerRef} className="flex-1 mb-6 overflow-y-scroll"> {/* ✅ added ref */}

                {messages.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center gap-2 mt-10">
                        <img
                            src={theme === "dark" ? assets.logo_full : assets.logo_full_dark}
                            alt="logo"
                            className="w-full max-w-[14rem] sm:max-w-[18rem]"
                        />
            ...
            {/* Prompt input box */}
            <form onSubmit={onSubmit} className='bg-primary/20 dark:bg-[#5BC0BE]/20 border border-primary
            dark:border-[#80609F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 items-center'>

                <select onChange={(e) => setMode(e.target.value)} value={mode} className='text-sm pl-3 pr-2 outline-none '>
                    <option className='bg-primary/20 dark:bg-primary' value='text'>Text</option>
                    <option className='bg-primary/20 dark:bg-primary' value='image'>Image</option>
                </select>
